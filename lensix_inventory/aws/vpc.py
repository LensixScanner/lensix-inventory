...
gateways, network ACLs, VPC endpoints, peering connections.
...
def gather(region, writer):
    flow_logs_by_vpc = {}
    for fl in get_flow_logs(region):
    ...
    for rt in get_route_tables(region):
        writer.add_resource(
    ...
    for nat in get_nat_gateways(region):
        writer.add_resource(
    ...
    for igw in get_internet_gateways(region):
    ...
    for vgw in get_vpn_gateways(region):
    ...
    for nacl in get_network_acls(region):
        writer.add_resource(
    ...
    for ep in get_vpc_endpoints(region):
        writer.add_resource(
    ...
    for pc in get_peering_connections(region):
