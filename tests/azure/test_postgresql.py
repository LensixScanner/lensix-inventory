...
def _vnet_rule(subnet_id='/subscriptions/s1/.../subnets/pg-subnet'):
    rule = MagicMock()
...
class TestGatherEdges:
    def test_a_single_servers_vnet_rule_gets_an_in_subnet_edge(self):
        ...
        pg_client.server_administrators.list.return_value = []
        pg_client.configurations.list_by_server.return_value = []
        pg_client.virtual_network_rules.list_by_server.return_value = [_vnet_rule(subnet_id='subnet-1')]
        with patch('azure.mgmt.rdbms.postgresql.PostgreSQLManagementClient', return_value=pg_client), \
             patch('azure.mgmt.rdbms.postgresql_flexibleservers.PostgreSQLManagementClient', return_value=MagicMock(servers=MagicMock(list=MagicMock(return_value=[])))):
            m.gather('cred', 'sub-1', w)
        w.add_edge.assert_called_once_with(
            from_type='postgresql_server', from_id=server.id, to_type='subnet', to_id='subnet-1', relationship='in_subnet',
        )
    ...
    def test_a_single_servers_vnet_rules_list_failure_is_recorded_and_gather_continues(self):
        ...
        pg_client.server_administrators.list.return_value = []
        pg_client.configurations.list_by_server.return_value = []
        pg_client.virtual_network_rules.list_by_server.side_effect = RuntimeError('boom')
        with patch('azure.mgmt.rdbms.postgresql.PostgreSQLManagementClient', return_value=pg_client), \
             patch('azure.mgmt.rdbms.postgresql_flexibleservers.PostgreSQLManagementClient', return_value=MagicMock(servers=MagicMock(list=MagicMock(return_value=[])))):
            m.gather('cred', 'sub-1', w)
        assert any(c.kwargs['source'] == 'postgresql:vnet_rules:s1' for c in w.add_error.call_args_list)
    ...
    def test_a_flexible_servers_delegated_subnet_gets_an_in_subnet_edge(self):
        ...
        with patch('azure.mgmt.rdbms.postgresql.PostgreSQLManagementClient', return_value=pg_client), \
            ...
            m.gather('cred', 'sub-1', w)
        w.add_edge.assert_called_once_with(
            from_type='postgresql_flexible_server', from_id=flex_server.id, to_type='subnet', to_id='subnet-1', relationship='in_subnet',
        )

    def test_a_flexible_server_with_no_vnet_integration_gets_no_edge(self):
        w = MagicMock()
        flex_server = _server(rid='/subscriptions/s1/resourceGroups/my-rg/providers/Microsoft.DBforPostgreSQL/flexibleServers/fs1', name='fs1')
        pg_client = MagicMock()
        pg_client.servers.list.return_value = []
        flex_client = MagicMock()
        flex_client.servers.list.return_value = [flex_server]
